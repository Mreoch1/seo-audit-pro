# SEO Audit Pro - Pricing Tiers & Options

## Overview

SEO Audit Pro offers 4 pricing tiers designed to meet different needs, from small websites to enterprise-level audits. All audits are manually reviewed and delivered as detailed PDF reports via email.

---

## Pricing Tiers

### 🟢 Starter - Essential SEO Audit
**Price:** $19  
**Page Limit:** Up to 50 pages  
**Best For:** Small websites, blogs, landing pages

#### Included Features:
- Deep Crawl & JavaScript Rendering
- Core Web Vitals Analysis
- Technical SEO Audit
- On-Page SEO Report
- Content Quality & Readability
- Accessibility Checks
- Local SEO Signals (Basic)
- Schema Detection
- Broken Links & Redirect Chains
- Internal Linking Overview
- Branded PDF Report

#### Available Add-Ons:
- **Blank Report (Unbranded)** - $10
- **Additional Pages** - $5 per 50 pages
- **Competitor Gap Analysis** - $15 (3 competitor URLs)
- **Schema Deep-Dive** - $15 (detailed structured data validation)
- **Extra Keywords** - $1 per keyword

---

### 🔵 Standard - Complete Site Analysis
**Price:** $39  
**Page Limit:** Up to 200 pages  
**Best For:** Small to medium businesses, e-commerce sites  
**⭐ MOST POPULAR**

#### Included Features:
Everything in Starter, plus:
- Advanced Local SEO Analysis
- Full Schema Validation
- Mobile Responsiveness Diagnostics
- Thin Content & Duplicate Detection
- Keyword Extraction (NLP)
- Readability Diagnostics
- Security Checks (HTTPS, HSTS)
- Platform Detection
- Automated Fix Recommendations

#### Available Add-Ons:
- **Blank Report (Unbranded)** - $10
- **Additional Pages** - $5 per 50 pages
- **Competitor Gap Analysis** - $15 (3 competitor URLs)
- **Extra Keywords** - $1 per keyword

**Note:** Schema Deep-Dive is not available for Standard tier (already included in base features).

---

### 🟡 Professional - Deep-Dive Analysis
**Price:** $59  
**Page Limit:** Up to 500 pages  
**Best For:** Large websites, agencies, complex sites

#### Included Features:
Everything in Standard, plus:
- Multi-level Internal Link Mapping
- Crawl Diagnostics & Insights
- Enhanced Accessibility (WCAG)
- Full Keyword Opportunity Mapping
- Detailed Content Structure Map
- JS/CSS Payload Analysis
- Core Web Vitals Opportunity Report
- Priority Fix Action Plan

#### Available Add-Ons:
- **Blank Report (Unbranded)** - $10
- **Additional Pages** - $5 per 50 pages
- **Extra Keywords** - $1 per keyword

**Note:** 
- Competitor Gap Analysis is not available for Professional tier (upgrade to Agency for competitor analysis)
- Extra Keywords are included in Professional tier (unlimited keywords in Agency)

---

### 🟣 Agency / Enterprise - Full Competitive Suite
**Price:** $99  
**Page Limit:** Unlimited Pages  
**Best For:** Agencies, enterprise clients, competitive analysis

#### Included Features:
Everything in Professional, plus:
- Unlimited Keywords (no extra charges)
- 3 Competitor Crawls & Gap Analysis (included)
- Full Local SEO Suite
- Social Signals Audit
- JS Rendering Diagnostics
- Full Internal Link Graph
- Orphan Page Detection
- Duplicate URL Cleaning

#### Available Add-Ons:
- **Blank Report (Unbranded)** - **FREE** (optional, not automatically included)
- **Additional Competitor** - $10 (adds 1 additional competitor - 4th competitor, max 4 total)
- **Extra Crawl Depth** - $15 (for complex sites)

**Note:** 
- Additional Pages add-on not available (unlimited pages included)
- Extra Keywords add-on not available (unlimited keywords included)
- Competitor Gap Analysis included (3 competitors), can add 1 more with Additional Competitor add-on (max 4 total)

---

## Add-Ons Reference

### Blank Report (Unbranded)
- **Price:** $10 (Free for Agency tier)
- **Available For:** All tiers
- **Description:** Remove SEO Audit Pro branding - Perfect for agencies who want to white-label the report for their clients
- **Special Notes:** 
  - Optional for Agency tier (not automatically included)
  - If Agency tier users want branded reports, they simply don't select this add-on

### Additional Pages
- **Price:** $5 per 50 pages
- **Available For:** Starter, Standard, Professional
- **Description:** Add 50 more pages to your crawl beyond the tier's base limit
- **Not Available For:** Agency tier (unlimited pages included)

### Competitor Gap Analysis
- **Price:** $15
- **Available For:** Starter, Standard
- **Description:** Keyword gap & competitive strategy report (includes 3 competitor URLs)
- **Included In:** Agency tier (3 competitors included, can add more)
- **Not Available For:** Professional tier

### Schema Deep-Dive
- **Price:** $15
- **Available For:** Starter tier only
- **Description:** Detailed structured data validation & recommendations
- **Note:** Standard tier and above already include full schema validation in base features

### Extra Keywords
- **Price:** $1 per keyword
- **Available For:** Starter, Standard, Professional
- **Description:** Additional keyword analysis & research beyond tier limits
- **Included In:** Agency tier (unlimited keywords)

### Additional Competitor
- **Price:** $10 (for 1 additional competitor)
- **Available For:** Agency tier only
- **Description:** Add 1 additional competitor website (4th competitor) - Agency tier includes 3 competitors, this adds 1 more (max 4 total)
- **Note:** This is a fixed price for one additional competitor, not per competitor

### Extra Crawl Depth
- **Price:** $15
- **Available For:** Agency tier only
- **Description:** Increase crawl depth for complex sites with deep navigation structures

---

## Pricing Examples

### Example 1: Small Business (Starter + Add-Ons)
- **Tier:** Starter ($19)
- **Add-Ons:**
  - Blank Report ($10)
  - Competitor Gap Analysis ($15)
- **Total:** $44

### Example 2: E-commerce Site (Standard)
- **Tier:** Standard ($39)
- **Add-Ons:** None
- **Total:** $39

### Example 3: Agency Client (Professional + White Label)
- **Tier:** Professional ($59)
- **Add-Ons:**
  - Blank Report ($10)
  - Additional Pages: 2 × 50 pages ($10)
- **Total:** $79

### Example 4: Enterprise (Agency Tier)
- **Tier:** Agency ($99)
- **Add-Ons:**
  - Blank Report (FREE)
  - Additional Competitor ($10) - adds 4th competitor
  - Extra Crawl Depth ($15)
- **Total:** $124

---

## Important Notes

### What's Included vs. What's an Add-On
- **Included:** Features listed under each tier are part of the base price
- **Add-Ons:** Optional extras that can be added for additional cost
- **Free for Higher Tiers:** Some add-ons are included free in higher tiers (e.g., Blank Report is free for Agency)

### Tier Recommendations
- **Starter:** Best for small websites, blogs, or quick health checks
- **Standard:** Most popular choice for small to medium businesses
- **Professional:** Ideal for large sites needing deep analysis
- **Agency:** Perfect for agencies and competitive analysis needs

### Smart Tier Suggestions
The system will automatically suggest upgrading to a higher tier if:
- Your current tier + add-ons cost the same or more than a higher tier
- A higher tier includes features you're paying for as add-ons

### Payment & Delivery
- All payments processed securely via Stripe
- Reports delivered via email as PDF attachments
- Reports may go to spam/junk folder - please check if not received
- No automated dashboards - every audit is manually reviewed

---

## Questions?

For pricing questions or custom enterprise solutions, please contact us through the order form or check the FAQ section on the website.

